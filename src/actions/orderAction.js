...
export const createOrder = (order) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post("http://localhost:5070/api/v1/order/new", order, config);
...
export const myOrder = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("http://localhost:5070/api/v1/orders/me");
...
export const getAllOrders = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("http://localhost:5070/api/v1/admin/orders");
...
export const updateOrders = (id, order) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `http://localhost:5070/api/v1/admin/orders/${id}`,
      order,
      config
    );
    console.log(data);
...
export const deleteOrder = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:5070/api/v1/admin/orders/${id}`);
...
export const getOrderDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:5070/api/v1/order/${id}`);
